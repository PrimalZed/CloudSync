...
public partial class SyncRootViewModel : ObservableObject, IDisposable {
	private readonly ISyncRootRegistrar _registrar;
	private readonly IPipe _registrarPipe;
	...
	public SyncRootViewModel(
		...
		_registrarPipe = registrarPipe;
		registrarPipe.ReceivedMessage += RegistrarPipe_ReceivedMessage;
	...
	private async void RegistrarPipe_ReceivedMessage(object? sender, string e) {
		_logger.LogDebug("Received registrar message: {message}", e);
		await UpdateIsRegistered();
	}
	...
	public void Dispose() {
		_registrarPipe.ReceivedMessage -= RegistrarPipe_ReceivedMessage;
